...
class QPointNetEncoder(nn.Module):
    def __init__(self, global_feat=True, feature_transform=False, channel=3):
        super(QPointNetEncoder, self).__init__()
        # self.stn = STN3d(channel)
        ...
        self.conv3_r = torch.nn.Conv1d(128, 1024, 1)
        self.conv3_g = torch.nn.Conv1d(128, 1024, 1)
        self.conv3_b = torch.nn.Conv1d(128, 1024, 1)
    ...
    def forward(self, x):
        ...
        if D > 3:
            x = torch.cat([x, feature], dim=2)
        x = x.transpose(2, 1)
        x = F.relu(self.bn1(self.conv1_r(x) * self.conv1_g(x) + self.conv1_b(x.mul(x))))
        ...
        pointfeat = x
        x = F.relu(self.bn2(self.conv2_r(x) * self.conv2_g(x) + self.conv2_b(x.mul(x))))
        x = self.bn3(self.conv3_r(x) * self.conv3_g(x) + self.conv3_b(x.mul(x)))
        x = torch.max(x, 2, keepdim=True)[0]
        x = x.view(-1, 1024)
        if self.global_feat:
            return x, trans, trans_feat
