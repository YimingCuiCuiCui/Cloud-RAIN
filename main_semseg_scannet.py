...
from models.dgcnn import DGCNN_semseg_scannet
from models.dgcnn_qnn import QDGCNN_semseg_scannet
from models.pointnet import PointNet_semseg_scannet
from models.pointnet_qnn import QPointNet_semseg_scannet
from models.pointnet_plus import PointNet_plus_sem_seg_msg_scannet, PointNet_plus_sem_seg_ssg_scannet
from models.pointnet_plus_qnn import QPointNet_plus_sem_seg_ssg_scannet, QPointNet_plus_sem_seg_msg_scannet
from models.gacnet import GACNet_semseg_scannet
from models.gacnet_qnn import QGACNet_semseg_scannet
from models.ldgcnn import LDGCNN_semseg_scannet
from models.ldgcnn_qnn import QLDGCNN_semseg_scannet
from models.gsnet import GSNET_semseg_scannet
from models.gsnet_qnn import QGSNET_semseg_scannet
from models.pct import PCT_semseg_scannet
from models.pct_qnn import QPCT_semseg_scannet
from models.pointmlp import pointMLP_semseg_scannet
from models.pointmlp_qnn import QpointMLP_semseg_scannet
...
def train(args, io):
    train_loader = DataLoader(ScanNet(split='train', data_root='./data/scannet', num_point=args.num_points),
                              num_workers=8, batch_size=args.batch_size, shuffle=True, drop_last=True)
    test_loader = DataLoader(ScanNet(split='test', data_root='./data/scannet', num_point=args.num_points),
    ...
    if args.model == 'pointnet':
        model = PointNet_semseg_scannet(args).to(device)
        model_category = 10
    elif args.model == 'pointnet_qnn':
        model = QPointNet_semseg_scannet(args).to(device)
        model_category = 11
    elif args.model == 'pointnet_pls_ssg':
        model = PointNet_plus_sem_seg_ssg_scannet(args).to(device)
        model_category = 20
    elif args.model == 'pointnet_pls_ssg_qnn':
        model = QPointNet_plus_sem_seg_ssg_scannet(args).to(device)
        model_category = 21
    elif args.model == 'pointnet_pls_msg':
        model = PointNet_plus_sem_seg_msg_scannet(args).to(device)
        model_category = 40
    elif args.model == 'pointnet_pls_msg_qnn':
        model = QPointNet_plus_sem_seg_msg_scannet(args).to(device)
        model_category = 41
    elif args.model == 'dgcnn':
        model = DGCNN_semseg_scannet(args).to(device)
        model_category = 30
    elif args.model == 'dgcnn_qnn':
        model = QDGCNN_semseg_scannet(args).to(device)
        model_category = 31
    elif args.model == 'ldgcnn':
        model = LDGCNN_semseg_scannet(args).to(device)
        model_category = 50
    elif args.model == 'ldgcnn_qnn':
        model = QLDGCNN_semseg_scannet(args).to(device)
        model_category = 51
    elif args.model == 'gacnet':
        model = GACNet_semseg_scannet().to(device)
        model_category = 60
    elif args.model == 'gacnet_qnn':
        model = QGACNet_semseg_scannet().to(device)
        model_category = 61
    elif args.model == 'gsnet':
        model = GSNET_semseg_scannet(args).to(device)
        model_category = 70
    elif args.model == 'gsnet_qnn':
        model = QGSNET_semseg_scannet(args).to(device)
        model_category = 71
    elif args.model == 'pct':
        model = PCT_semseg_scannet(args).to(device)
        model_category = 80
    elif args.model == 'pct_qnn':
        model = QPCT_semseg_scannet(args).to(device)
        model_category = 81
    elif args.model == 'pointmlp':
        model = pointMLP_semseg_scannet(args).to(device)
        model_category = 90
    elif args.model == 'pointmlp_qnn':
        model = QpointMLP_semseg_scannet(args).to(device)
        model_category = 91
    else:
    ...
    for epoch in range(args.epochs):
        ...
        for data, seg in train_loader:
            ...
            if model_category // 10 == 1:
                ...
                loss = criterion(seg_pred.view(-1, 21), seg.view(-1,1).squeeze()) + reg_loss.mean()
            else:
                seg_pred = model(data)
                seg_pred = seg_pred.permute(0, 2, 1).contiguous()
                loss = criterion(seg_pred.view(-1, 21), seg.view(-1,1).squeeze())
            loss.backward()
            opt.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='Scannet', metavar='N',
                        choices=['Scannet'])
