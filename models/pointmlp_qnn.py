...
import sys
sys.path.append("..")
from  utils.pointmlp_util import ConvBNReLU1D, QPosExtraction, QPreExtraction, \
    PointNetFeaturePropagation, LocalGrouper, get_activation

class QPointMLP(nn.Module):
    def __init__(self, num_classes=50,points=2048, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        for i in range(len(pre_blocks)):
            ...
            pre_block_module = QPreExtraction(last_channel, out_channel, pre_block_num, groups=groups,
                                             res_expansion=res_expansion,
                                             bias=bias, activation=activation, use_xyz=use_xyz)
            self.pre_blocks_list.append(pre_block_module)
            # append pos_block_list
            pos_block_module = QPosExtraction(out_channel, pos_block_num, groups=groups,
...
def QpointMLP_semseg_s3dis(args, num_classes=13, **kwargs) -> QPointMLP:
    return QPointMLP(num_classes=num_classes, points=4096, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
...
def QpointMLP_semseg_scannet(args, num_classes=21, **kwargs) -> QPointMLP:
    return QPointMLP(num_classes=num_classes, points=8192, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
                 de_dims=[512, 256, 128, 128], de_blocks=[4, 4, 4, 4],
...
if __name__ == '__main__':
    ...
    model = QpointMLP_semseg_s3dis(args, 13)
    ...
    parser = argparse.ArgumentParser(description='Point Cloud Part Segmentation')
    args = parser.parse_args()
    model = QpointMLP_semseg_scannet(args, 21)
    out = model(data)  # [2,2048,50]
    print(out.shape)
