...
from datasets.shapenet import ShapeNetPart

from models.dgcnn import DGCNN_partseg
from models.dgcnn_qnn import QDGCNN_partseg
from models.pointnet import PointNet_partseg
from models.pointnet_qnn import QPointNet_partseg
from models.pointnet_plus import PointNet_plus_part_seg_msg, PointNet_plus_part_seg_ssg
from models.pointnet_plus_qnn import QPointNet_plus_part_seg_msg, QPointNet_plus_part_seg_ssg
...
def train(args, io):
    ...
    #                 20 -> PointNet++
    #                 30 -> DGCNN
    # ['dgcnn', 'dgcnn_qnn', 'pointnet', 'pointnet_qnn', 'pointnet_pls_ssg_qnn',
     # 'pointnet_pls_msg', 'pointnet_pls_ssg', 'pointnet_pls_msg_qnn'],

    #Try to load models
    if args.model == 'pointnet':
        model = PointNet_partseg(args, seg_num_all).to(device)
        model_category = 10
    elif args.model == 'pointnet_qnn':
        model = QPointNet_partseg(args, seg_num_all).to(device)
        model_category = 11
    elif args.model == 'pointnet_pls_ssg':
        model = PointNet_plus_part_seg_ssg(args, seg_num_all).to(device)
        model_category = 20
    elif args.model == 'pointnet_pls_ssg_qnn':
        model = QPointNet_plus_part_seg_ssg(args, seg_num_all).to(device)
        model_category = 21
    elif args.model == 'pointnet_pls_msg':
        model = PointNet_plus_part_seg_msg(args, seg_num_all).to(device)
        model_category = 40
    elif args.model == 'pointnet_pls_msg_qnn':
        model = QPointNet_plus_part_seg_msg(args, seg_num_all).to(device)
        model_category = 41
    elif args.model == 'dgcnn':
        model = DGCNN_partseg(args, seg_num_all).to(device)
        model_category = 30
    elif args.model == 'dgcnn_qnn':
        model = QDGCNN_partseg(args, seg_num_all).to(device)
        model_category = 31
    else:
    ...
    for epoch in range(args.epochs):
        ...
        for data, label, seg in train_loader:
            ...
            if model_category // 10 == 1:
                seg_pred, reg_loss = model(data, label_one_hot)
                seg_pred = seg_pred.permute(0, 2, 1).contiguous()
                loss = criterion(seg_pred.view(-1, seg_num_all), seg.view(-1, 1).squeeze()) + reg_loss
            else:
                seg_pred = model(data, label_one_hot)
                seg_pred = seg_pred.permute(0, 2, 1).contiguous()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='dgcnn', metavar='N',
                        choices=['dgcnn', 'dgcnn_qnn', 'pointnet', 'pointnet_qnn', 'pointnet_pls_ssg_qnn',
                                 'pointnet_pls_msg', 'pointnet_pls_ssg', 'pointnet_pls_msg_qnn'],
